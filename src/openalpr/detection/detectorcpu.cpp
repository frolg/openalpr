...
namespace alpr
{
  ...
  DetectorCPU::DetectorCPU(Config* config, PreWarp* prewarp) : Detector(config, prewarp) {
    ...
    this->plate_cascade_RU.load( get_detector_file_EU());
  }
...
//		  plate_cascade_RU.detectMultiScale( frame, plates, config->detection_iteration_increase, config->detectionStrictness,
//		                                        CV_HAAR_DO_CANNY_PRUNING,
//		                                        //0|CV_HAAR_SCALE_IMAGE,
//		                                        min_plate_size, max_plate_size );
//		if (config->debugGeneral)
